#Kirara Fantasia代肝脚本(BlueStacks)Ver.4.3.1.210525
# -*- coding: utf-8 -*-
#@author:EnderLop
import numpy as np
...
#全局变量声明
TOTAL_ROUND = 0
WINDOW_ID,MAINWINDOW_ID1,MAINWINDOW_ID2 = 0,0,0
...
def get_child_windows():
    global WINDOW_ID,MAINWINDOW_ID1,MAINWINDOW_ID2
    if not WINDOW_ID:
        return None
    child_list = []
    win32gui.EnumChildWindows(WINDOW_ID,lambda hwnd,param:param.append(hwnd),child_list)
    for _suspect_target in child_list:
        if "BlueStacks Android PluginAndroid" in win32gui.GetWindowText(_suspect_target) or "plrNativeInputWindow" in win32gui.GetWindowText(_suspect_target):
            MAINWINDOW_ID1 = _suspect_target
        if win32gui.GetWindowText(_suspect_target) == "_ctl.Window":
            MAINWINDOW_ID2 = _suspect_target
            break
    else:
        print("未找到游戏句柄")
...
'''游戏主窗口句柄应用'''
def state_change():
    global WIDTH,HEIGHT,MAINWINDOW_ID2
    _four_state = win32gui.GetWindowRect(MAINWINDOW_ID2)
    WIDTH,HEIGHT = _four_state[2] - _four_state[0],_four_state[3] - _four_state[1]

'''1080P→窗口化绝对位置调整'''
def state_transform1(x_origin,y_origin):
    global WIDTH,HEIGHT
...
def state_transform(x_origin,y_origin):
    global WIDTH,HEIGHT,MAINWINDOW_ID1,MAINWINDOW_ID2
    _four_state1 = win32gui.GetWindowRect(MAINWINDOW_ID1)
    _four_state2 = win32gui.GetWindowRect(MAINWINDOW_ID2)
    x_new = _four_state2[0] - _four_state1[0] + x_origin / 1920 * WIDTH#X轴绝对位置变换
    y_new = _four_state2[1] - _four_state1[1] + y_origin / 1080 * HEIGHT#Y轴绝对位置变换
    return int(x_new),int(y_new)
...
def detect_flash():
    global WIDTH,HEIGHT,MAINWINDOW_ID2
    hWndDC = win32gui.GetWindowDC(MAINWINDOW_ID2)
    ...
    saveDC.DeleteDC()
    mfcDC.DeleteDC()
    win32gui.ReleaseDC(MAINWINDOW_ID2,hWndDC)
    return color_info

'''判断当前位置颜色是否含有特殊信息'''
def color_compare(info,state_x,state_y,R,G,B):
    if info[state_transform1(state_x,state_y)].tolist() == [R,G,B]:
        return True
    else:
        return False
...
def mouse_click(state_x,state_y,n):
    global MAINWINDOW_ID1
    tmp = win32api.MAKELONG(state_x,state_y)
    for i in range(n):
        win32gui.SendMessage(MAINWINDOW_ID1,win32con.WM_ACTIVATE,win32con.WA_ACTIVE,0)
        time.sleep(0.1)
        win32api.PostMessage(MAINWINDOW_ID1,win32con.WM_LBUTTONDOWN,win32con.MK_LBUTTON,tmp)
        time.sleep(0.1)
        win32api.PostMessage(MAINWINDOW_ID1,win32con.WM_LBUTTONUP,0000,tmp)
