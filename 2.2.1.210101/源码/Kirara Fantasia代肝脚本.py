# Kirara Fantasia代肝脚本Ver.2.2.1.210101
# -*- coding: utf-8 -*-
# @author:EnderLop
...
def corner_accurater():
    global WIDTH, HEIGHT, POSITION, COLOR
    info = detect_flash()
    for j in range(max(POSITION[1] - 10, 0), min(POSITION[1] + 10, 1079)):
        for i in range(max(POSITION[0] - 10, 0), min(POSITION[0] + 10, 1919)):
            if not info[i, j].tolist() == COLOR:
                POSITION[0], POSITION[1] = i, j
    for j in range(min(POSITION[3] + 10, 1079), max(POSITION[3] - 10, 0), -1):
        for i in range(min(POSITION[2] + 10, 1919), max(POSITION[2] - 10, 0), -1):
            if not info[i, j].tolist() == COLOR:
